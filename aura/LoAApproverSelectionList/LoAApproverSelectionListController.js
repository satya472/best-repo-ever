...
({
    init: function (cmp) {
        ...
        */
        ...
        //helper.preselect(cmp, event, helper);
    ...
    getSelectedName: function (cmp, event) {
        ...
        var rowSelections = event.getParam('selectedRows');

        //cmp.set("v.selectedRows", rowSelections);
        var approverUserIds =[];
        for (var i = 0; i < rowSelections.length; i++){
            approverUserIds.push(rowSelections[i].UserId__c);
            //alert(approverUserIds);
        }
        console.log(approverUserIds);
        cmp.set('v.approverIds', approverUserIds);
    }
})